...
* @file FOC_ExternalInterface_NoAI.h
...
#ifndef _FOC_EXTERNAL_INTERFACE_NO_AI_H_

 

#define _FOC_EXTERNAL_INTERFACE_NO_AI_H_
...
#include "FOC_Diagnose.h"

 

#include "FOC_Simplify.h"
...
函数名称: Foc_SetVoltageReference

 

函数功能: 设置电机以电压控制闭环控制运行，d，q轴目标参考电压,Vd, Vq设置范围[-20V, +20V]
...
函数名称: Foc_SetIFReference

 

函数功能: 设置电机以IF控制闭环控制运行
...
输出参数： pfThetaElec：电机当前电角度，°

 

pfSpeed：电机当前机械角速度指针，RPM，正数表示逆时针转，负数表示顺时针转
...
输出参数: pstHallStatesOffset：当前电机霍尔补偿距离状态指针
...
函数名称: Foc_WriteMotorHallStates

 

函数功能: 写入电机当前霍尔状态 修改补偿后的霍尔行程
...
输出参数: pstHallStatesOffset：当前状态霍尔行程

 

nHallDistanceOffset: 当前电机霍尔行程补偿值
