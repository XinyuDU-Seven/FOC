...
#define FOC_HALL_TRAVEL_STALL_REASON_NONE      0U
#define FOC_HALL_TRAVEL_STALL_REASON_PROGRESS  1U
#define FOC_HALL_TRAVEL_STALL_REASON_NO_EDGE   2U
#define FOC_HALL_TRAVEL_STALL_REASON_SPEED_ERR 3U
...
FOC_DEBUG_ROOT volatile uint16_t g_foc_endpoint_release_elapsed_ms = 0U;
...
static uint8_t  s_endpoint_release_active_store[FOC_CORE_MOTOR_COUNT] = {0U, 0U};
static uint8_t  s_endpoint_release_timeout_store[FOC_CORE_MOTOR_COUNT] = {0U, 0U};
static uint8_t  s_endpoint_release_edge_count_store[FOC_CORE_MOTOR_COUNT] = {0U, 0U};
static uint8_t  s_endpoint_release_last_sector_store[FOC_CORE_MOTOR_COUNT] = {0U, 0U};
static FOC_Dir_e s_endpoint_release_direction_store[FOC_CORE_MOTOR_COUNT] = {
    FOC_DIR_CW,
    FOC_DIR_CW
};
...
FOC_DEBUG_ROOT volatile uint16_t g_foc_motor0_cw_breakaway_iq_mA = 5000U;
FOC_DEBUG_ROOT volatile uint16_t g_foc_motor0_ccw_breakaway_iq_mA = 5000U;
...
FOC_DEBUG_ROOT volatile uint8_t
    g_foc_motor1_speed_start_bumpless_enable = 1U;
FOC_DEBUG_ROOT volatile uint16_t
...
#define s_endpoint_release_active       (s_endpoint_release_active_store[s_foc_core_active_motor])
#define s_endpoint_release_timeout      (s_endpoint_release_timeout_store[s_foc_core_active_motor])
#define s_endpoint_release_edge_count   (s_endpoint_release_edge_count_store[s_foc_core_active_motor])
#define s_endpoint_release_last_sector  (s_endpoint_release_last_sector_store[s_foc_core_active_motor])
...
static uint8_t FOC_SpeedStart_Motor1BumplessEnabled(void)
{
     return ((s_foc_core_active_motor == 1U) &&
             (g_foc_motor1_speed_start_bumpless_enable != 0U)) ? 1U : 0U;
}

static uint8_t FOC_SpeedStart_HandoffMinElapsed(void)
{
     uint32_t handoff_min_us =
         (uint32_t)g_foc_speed_start_handoff_ms * 1000U;

     return ((handoff_min_us == 0U) ||
             (s_speed_start_handoff_elapsed_us >= handoff_min_us)) ? 1U : 0U;
}

static uint8_t FOC_SpeedStart_Motor1CatchupTimedOut(void)
{
     uint32_t timeout_us;
...
static void FOC_SpeedStart_AdvanceHandoffElapsed(uint32_t elapsed_us)
{
     if ((0xFFFFFFFFU - s_speed_start_handoff_elapsed_us) >= elapsed_us) {
         s_speed_start_handoff_elapsed_us += elapsed_us;
     } else {
                                               ...
                                               float speed_dt)
{
     ...
     uint8_t motor1_bumpless =
         FOC_SpeedStart_Motor1BumplessEnabled();
     uint8_t catchup_active =
         FOC_SpeedStart_ClosedCatchupActive(speed_ref_ctrl, speed_error);
     ...
     if (motor1_bumpless == 0U) {
         FOC_SpeedStart_AdvanceHandoffElapsed(elapsed_step_us);
     }
     ...
     if (motor1_bumpless != 0U) {
         ...
         if ((FOC_SpeedStart_HandoffMinElapsed() != 0U) &&
             ...
             g_foc_motor1_speed_start_bumpless_active = 1U;
...
static void FOC_EndpointRelease_UpdateDebug(void)
{
    ...
    g_foc_endpoint_release_elapsed_ms =
        FOC_SpeedStartElapsedMs(s_endpoint_release_elapsed_us);
...
static uint8_t FOC_EndpointReleaseActive(void)
{
    return s_endpoint_release_active;
}

static uint8_t FOC_EndpointReleaseHardInactive(void)
{
    return ((g_foc_motor0_endpoint_release_enable == 0U) ||
            (s_foc_core_active_motor != 0U) ||
            (s_ctx.state != FOC_STATE_RUNNING) ||
            (s_foc_ctrl_source != FOC_CTRL_SOURCE_SPEED) ||
            (s_ctx.direction > FOC_DIR_CCW) ||
            (FOC_SpeedStart_CommandActive(s_speed_ref_ctrl) == 0U) ||
                                         ...
                                         uint32_t period_us)
{
    ...
    if (s_endpoint_release_direction != s_ctx.direction) {
        FOC_EndpointRelease_Reset(1U);
    }
    ...
    if (s_endpoint_release_active != 0U) {
        ...
        if ((cur_sector != 0U) &&
            (s_endpoint_release_last_sector != 0U) &&
            (cur_sector != s_endpoint_release_last_sector)) {
            edge_seen = 1U;
        ...
        if (FOC_EndpointReleaseStartEligible() == 0U) {
            FOC_EndpointRelease_UpdateDebug();
            return theta_ctrl;
        }

        s_endpoint_release_active = 1U;
        s_endpoint_release_direction = s_ctx.direction;
        s_endpoint_release_edge_count = 0U;
        s_endpoint_release_last_sector = cur_sector;
        s_endpoint_release_elapsed_us = 0U;
    ...
    if ((0xFFFFFFFFU - s_endpoint_release_elapsed_us) >= period_us) {
        s_endpoint_release_elapsed_us += period_us;
    } else {
    ...
    if (dt <= 0.0f) {
        dt = (float)FOC_CONTROL_PERIOD_US * 1.0e-6f;
    }
    if (release_rpm < 1.0f) {
        release_rpm = 1.0f;
    }
    ...
    return s_endpoint_release_angle;
}
...
static void FOC_UpdateHallTravelStallGuard(void)
{
    ...
    float target_margin = target_abs * 0.02f;
    float current_min =
    ...
    uint8_t endpoint_release_active = FOC_EndpointReleaseActive();
    ...
    if (target_margin < 5.0f) {
        target_margin = 5.0f;
    }
    if ((target_abs < 1.0f) ||
        ((ref_abs + target_margin) >= target_abs)) {
        target_ready = 1U;
    }
    if (speed_err_threshold == 0U) {
        speed_err_threshold = 1U;
    }
    if ((threshold != 0U) && (threshold < speed_err_threshold)) {
        speed_err_threshold = threshold;
    }
    if ((target_ready != 0U) &&
    ...
    if ((g_foc_hall_travel_stall_guard_enable != 0U) &&
        ...
        (command_sign != 0)) {
        if (s_hall_travel_speed_err_counter < speed_err_threshold) {
            s_hall_travel_speed_err_counter++;
        }
    } else if ((g_foc_hall_travel_stall_guard_enable == 0U) ||
               (s_ctx.state != FOC_STATE_RUNNING) ||
               (s_foc_ctrl_source != FOC_CTRL_SOURCE_SPEED) ||
               (ref_abs < (float)g_foc_hall_travel_stall_min_ref_rpm) ||
               (endpoint_release_active != 0U) ||
               ...
               (command_sign == 0)) {
        s_hall_travel_speed_err_counter = 0U;
    } else if (s_hall_travel_speed_err_counter > 0U) {
        s_hall_travel_speed_err_counter--;
    }
    if (s_hall_travel_speed_err_counter >= speed_err_threshold) {
        speed_error_stall = 1U;
    }
    ...
    if (command_sign == 0) {
        ...
        if ((s_hall_travel_stall_active != 0U) &&
            ...
            s_hall_travel_stall_dir = 0;
            s_hall_travel_stall_transient = 0U;
            s_hall_travel_stall_reason =
                FOC_HALL_TRAVEL_STALL_REASON_NONE;
            FOC_UpdateHallTravelStallDebug();
            return;
        }
    ...
    if (condition != 0U) {
        ...
        if (s_hall_travel_stall_counter >= threshold) {
            ...
            if ((was_active == 0U) &&
                (persistent_stall_allowed != 0U) &&
                (s_hall_travel_stall_reason !=
                 FOC_HALL_TRAVEL_STALL_REASON_SPEED_ERR)) {
                                    ...
                                    uint8_t allow_missed_transition)
{
...

 {
     ...
     theta_e_ctrl = FOC_EndpointRelease_Service(theta_e_ctrl,
                                                observer_dt,
                                                control_period_us);
