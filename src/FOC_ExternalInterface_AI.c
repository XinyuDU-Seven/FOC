...
extern volatile uint8_t  g_foc_vbus_brake_active;
...
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_sethybrid_speed_zero_deadband_rpm = 10U;
 ...
 * 3 sethybrid +/-1000rpm sine, 3s period, 4 -1000..-4000 sine.
 */
...
FOC_AI_DEBUG_ROOT volatile uint8_t  g_foc_test_case_first_cycle_log_done = 0U;
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_test_case_first_cycle_log_idx = 0U;
FOC_AI_DEBUG_ROOT volatile uint8_t  g_foc_test_case_fixed_speed_log_enable = 1U;
FOC_AI_DEBUG_ROOT volatile uint8_t  g_foc_test_case_fixed_speed_log_reset = 0U;
FOC_AI_DEBUG_ROOT volatile uint8_t  g_foc_test_case_fixed_speed_log_active = 0U;
...
FOC_AI_DEBUG_ROOT volatile int16_t  g_foc_test_case_fixed_speed_log_filtered_rpm[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile int16_t  g_foc_test_case_fixed_speed_log_iq_ref_mA[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile int16_t  g_foc_test_case_fixed_speed_log_iq_mA[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_test_case_fixed_speed_log_no_edge_limit_rpm[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_test_case_fixed_speed_log_sector_no_change[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_test_case_fixed_speed_log_flags[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_test_case_fixed_speed_log_hall_dt_us[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_test_case_fixed_speed_log_no_edge_elapsed_us[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_test_case_fixed_speed_log_hall_event_seq[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_test_case_fixed_speed_log_hall_debug[FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE];
...
FOC_AI_DEBUG_ROOT volatile int16_t  g_foc_test_case_first_cycle_log_actual_rpm[FOC_TEST_CASE_FIRST_CYCLE_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile int16_t  g_foc_test_case_first_cycle_log_filtered_rpm[FOC_TEST_CASE_FIRST_CYCLE_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile int16_t  g_foc_test_case_first_cycle_log_iq_ref_mA[FOC_TEST_CASE_FIRST_CYCLE_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile int16_t  g_foc_test_case_first_cycle_log_iq_mA[FOC_TEST_CASE_FIRST_CYCLE_LOG_SIZE];
FOC_AI_DEBUG_ROOT volatile uint16_t g_foc_test_case_first_cycle_log_vbus_mV[FOC_TEST_CASE_FIRST_CYCLE_LOG_SIZE];
...
static uint8_t s_foc_test_case_fixed_speed_log_decim_count = 0U;
static uint8_t s_foc_test_case_fixed_speed_log_last_nonzero = 0U;
static uint8_t s_foc_test_case_first_cycle_log_decim_count = 0U;
...
static void FOC_TestCase_RecordFirstCycleLog(void);
...
void Foc_AlgorithmControlCallback_AI(void){
  ...
  FOC_TestCase_RecordFirstCycleLog();
...
static uint16_t FOC_TestCase_AbsRpmToU16(float rpm)
{
  float abs_rpm = FOC_FABS(rpm);

  if (abs_rpm > 65535.0f) {
    return 65535U;
  }

  return (uint16_t)(abs_rpm + 0.5f);
}

static uint16_t FOC_TestCase_PhaseToU16(float phase)
{
  float scaled = phase * (65535.0f / FOC_2PI);
...
static uint16_t FOC_TestCase_FloatToU16(float value, float scale)
{
  float scaled = value * scale;
...
static uint16_t FOC_TestCase_U32ToU16(uint32_t value)
{
...
static void FOC_TestCase_ClearFixedSpeedLog(void)
{
  uint16_t idx;

  g_foc_test_case_fixed_speed_log_active = 0U;
  g_foc_test_case_fixed_speed_log_done = 0U;
  g_foc_test_case_fixed_speed_log_idx = 0U;
  s_foc_test_case_fixed_speed_log_decim_count = 0U;
  ...
  for (idx = 0U; idx < FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE; idx++) {
    ...
    g_foc_test_case_fixed_speed_log_filtered_rpm[idx] = 0;
    g_foc_test_case_fixed_speed_log_iq_ref_mA[idx] = 0;
    g_foc_test_case_fixed_speed_log_iq_mA[idx] = 0;
    g_foc_test_case_fixed_speed_log_no_edge_limit_rpm[idx] = 0U;
    g_foc_test_case_fixed_speed_log_sector_no_change[idx] = 0U;
    g_foc_test_case_fixed_speed_log_flags[idx] = 0U;
    g_foc_test_case_fixed_speed_log_hall_dt_us[idx] = 0U;
    g_foc_test_case_fixed_speed_log_no_edge_elapsed_us[idx] = 0U;
    g_foc_test_case_fixed_speed_log_hall_event_seq[idx] = 0U;
    g_foc_test_case_fixed_speed_log_hall_debug[idx] = 0U;
  }
}

static void FOC_TestCase_StartFixedSpeedLog(void)
{
  FOC_TestCase_ClearFixedSpeedLog();

  if (g_foc_test_case_fixed_speed_log_enable != 0U) {
    g_foc_test_case_fixed_speed_log_active = 1U;
  }
}

static void FOC_TestCase_ServiceFixedSpeedLog(void)
{
  const FOC_Context_t *ctx;
  int16_t target_rpm = FOC_AI_SpeedLogToI16(gfSpeedTarget);
  uint8_t target_nonzero = (target_rpm != 0) ? 1U : 0U;
  uint16_t idx;
  uint16_t flags = 0U;

  if (g_foc_test_case_fixed_speed_log_reset != 0U) {
    FOC_TestCase_ClearFixedSpeedLog();
    g_foc_test_case_fixed_speed_log_reset = 0U;
    s_foc_test_case_fixed_speed_log_last_nonzero = 0U;
  }

  if (target_nonzero == 0U) {
    g_foc_test_case_fixed_speed_log_active = 0U;
    s_foc_test_case_fixed_speed_log_decim_count = 0U;
    s_foc_test_case_fixed_speed_log_last_nonzero = 0U;
    return;
  ...
  if (s_foc_test_case_fixed_speed_log_last_nonzero == 0U) {
    FOC_TestCase_StartFixedSpeedLog();
  }
  s_foc_test_case_fixed_speed_log_last_nonzero = 1U;
  ...
  s_foc_test_case_fixed_speed_log_decim_count++;
  if (s_foc_test_case_fixed_speed_log_decim_count <
      FOC_TEST_CASE_FIXED_SPEED_LOG_DECIMATION) {
    return;
  }
  s_foc_test_case_fixed_speed_log_decim_count = 0U;

  idx = g_foc_test_case_fixed_speed_log_idx;
  if (idx >= FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE) {
    g_foc_test_case_fixed_speed_log_active = 0U;
    g_foc_test_case_fixed_speed_log_done = 1U;
    return;
  }
  ...
  if (g_foc_vbus_brake_active != 0U) {
    flags |= 0x0002U;
  }
  if (ctx->fault != FOC_FAULT_NONE) {
    flags |= 0x0004U;
  }
  if (ctx->direction == FOC_DIR_CCW) {
    flags |= 0x0008U;
  }
  if (g_foc_speed_drop_fault_count != 0U) {
    flags |= 0x0010U;
  }
  if (FOC_FABS((float)target_rpm) <=
      (float)g_foc_sethybrid_speed_zero_deadband_rpm) {
    flags |= 0x0020U;
  }
  ...
  g_foc_test_case_fixed_speed_log_actual_rpm[idx] =
      FOC_AI_SignedRawSpeed(ctx);
  g_foc_test_case_fixed_speed_log_filtered_rpm[idx] =
      FOC_AI_SignedSpeedByRawDirection(ctx, ctx->speed_ctrl_fdb);
  g_foc_test_case_fixed_speed_log_iq_ref_mA[idx] =
      FOC_AI_SpeedLogToI16(FOC_AI_SignedIqRef(ctx) * 1000.0f);
  g_foc_test_case_fixed_speed_log_iq_mA[idx] =
      FOC_AI_SpeedLogToI16(ctx->i_dq.q * 1000.0f);
  g_foc_test_case_fixed_speed_log_no_edge_limit_rpm[idx] =
      g_foc_observer_no_edge_speed_limit_rpm;
  g_foc_test_case_fixed_speed_log_sector_no_change[idx] =
      FOC_TestCase_U32ToU16(ctx->sector_no_change_count);
  g_foc_test_case_fixed_speed_log_flags[idx] = flags;
  g_foc_test_case_fixed_speed_log_hall_dt_us[idx] =
      FOC_TestCase_U32ToU16(ctx->hall_sector_dt_us);
  g_foc_test_case_fixed_speed_log_no_edge_elapsed_us[idx] =
      FOC_TestCase_U32ToU16(g_foc_observer_no_edge_elapsed_us);
  g_foc_test_case_fixed_speed_log_hall_event_seq[idx] =
      (uint16_t)g_foc_hall_event_seq;
  g_foc_test_case_fixed_speed_log_hall_debug[idx] =
      FOC_TestCase_PackHallDebug(ctx);

  idx++;
  g_foc_test_case_fixed_speed_log_idx = idx;
  if (idx >= FOC_TEST_CASE_FIXED_SPEED_LOG_SIZE) {
    g_foc_test_case_fixed_speed_log_active = 0U;
    g_foc_test_case_fixed_speed_log_done = 1U;
  }
...
static void FOC_TestCase_ServiceFixedSpeed(void)
{
  ...
  FOC_TestCase_ServiceFixedSpeedLog();
...
static void FOC_TestCase_ClearFirstCycleLog(void)
{
  ...
  g_foc_test_case_first_cycle_log_done = 0U;
  g_foc_test_case_first_cycle_log_idx = 0U;
  s_foc_test_case_first_cycle_log_decim_count = 0U;
}

static void FOC_TestCase_StartFirstCycleLog(void)
{
  FOC_TestCase_ClearFirstCycleLog();

  if (g_foc_test_case_first_cycle_log_enable != 0U) {
    g_foc_test_case_first_cycle_log_active = 1U;
...
static void FOC_TestCase_RecordFirstCycleLog(void)
{
  ...
  if (g_foc_test_case_first_cycle_log_reset != 0U) {
    FOC_TestCase_ClearFirstCycleLog();
    g_foc_test_case_first_cycle_log_reset = 0U;
    if ((g_foc_test_case_first_cycle_log_enable != 0U) &&
  ...
  s_foc_test_case_first_cycle_log_decim_count++;
  if (s_foc_test_case_first_cycle_log_decim_count <
      FOC_TEST_CASE_FIRST_CYCLE_LOG_DECIMATION) {
    return;
  }
  s_foc_test_case_first_cycle_log_decim_count = 0U;

  idx = g_foc_test_case_first_cycle_log_idx;
  if (idx >= FOC_TEST_CASE_FIRST_CYCLE_LOG_SIZE) {
    g_foc_test_case_first_cycle_log_active = 0U;
    g_foc_test_case_first_cycle_log_done = 1U;
    return;
  }
  ...
  if (FOC_FABS((float)g_foc_bidir_speed_ref_rpm) <=
      (float)g_foc_sethybrid_speed_zero_deadband_rpm) {
    flags |= 0x0020U;
  }

  g_foc_test_case_first_cycle_log_t_ms[idx] =
      FOC_TestCase_U32ToU16(elapsed_ms);
  ...
  g_foc_test_case_first_cycle_log_iq_mA[idx] =
      FOC_AI_SpeedLogToI16(ctx->i_dq.q * 1000.0f);
  g_foc_test_case_first_cycle_log_vbus_mV[idx] =
      FOC_TestCase_FloatToU16(ctx->v_bus, 1000.0f);
  g_foc_test_case_first_cycle_log_no_edge_limit_rpm[idx] =
      g_foc_observer_no_edge_speed_limit_rpm;
...
static void FOC_TestCase_StartSetHybridSine(void)
{
  s_foc_test_case_sethybrid_sine_active = 1U;
  s_foc_test_case_sethybrid_sine_decim_count =
      FOC_TEST_CASE_SETHYBRID_SINE_DECIMATION - 1U;
  s_foc_test_case_sethybrid_sine_start_us = FOC_HAL_GetTimestampUs();

  g_foc_bidir_speed_period_ms = FOC_TEST_CASE_SETHYBRID_SINE_PERIOD_MS;
  g_foc_bidir_speed_max_rpm = FOC_TEST_CASE_SETHYBRID_SINE_AMP_RPM;
  g_foc_bidir_speed_step_enable = 0U;
  g_foc_bidir_speed_enable = 0U;
  g_foc_bidir_speed_reset_stats = 0U;
  g_foc_bidir_speed_elapsed_ms = 0U;
  g_foc_bidir_speed_phase_u16 = 0U;
  g_foc_bidir_speed_ref_rpm = 0;

  g_foc_dyn_speed_elapsed_ms = 0U;
  g_foc_dyn_speed_phase_u16 = 0U;
  g_foc_dyn_speed_ref_rpm = 0;
  g_foc_dyn_speed_last_ext_ref_rpm = 0;

  FOC_TestCase_StartFirstCycleLog();
}
...
static void FOC_TestCase_ServiceSetHybridSine(void)
{
  ...
  uint32_t elapsed_us;
  uint32_t period_us =
      FOC_TEST_CASE_SETHYBRID_SINE_PERIOD_MS * 1000U;
  uint32_t phase_us;
  float phase;
  float target;
  FocError err;
  ...
  now_us = FOC_HAL_GetTimestampUs();
  elapsed_us = now_us - s_foc_test_case_sethybrid_sine_start_us;
  phase_us = (period_us > 0U) ? (elapsed_us % period_us) : 0U;
  phase = ((float)phase_us / (float)period_us) * FOC_2PI;
  target = (float)FOC_TEST_CASE_SETHYBRID_SINE_AMP_RPM * FOC_FastSin(phase);

  g_foc_bidir_speed_elapsed_ms = elapsed_us / 1000U;
  g_foc_bidir_speed_phase_u16 = FOC_TestCase_PhaseToU16(phase);
  g_foc_bidir_speed_ref_rpm = FOC_AI_SpeedLogToI16(target);
  ...
  s_foc_test_case_sethybrid_sine_decim_count++;
  if (s_foc_test_case_sethybrid_sine_decim_count <
      FOC_TEST_CASE_SETHYBRID_SINE_DECIMATION) {
    return;
  }
...
static void FOC_TestCase_Apply(uint8_t test_case)
{
  ...
  if(test_case == FOC_TEST_CASE_STOP){
    ...
    FOC_TestCase_StartSetHybridSine();
